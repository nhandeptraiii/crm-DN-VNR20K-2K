...
public interface AppointmentRepository extends JpaRepository<Appointment, Long> {
        ...
        /**
         * Tìm lịch hẹn cần gửi email nhắc trước 24h. Điều kiện: - status không phải CONFIRMED /
         * CANCELLED - reminder_24h_sent = false - scheduledTime nằm trong khoảng (from, to) — tức
         * là (now+23h, now+25h)
         */
        ...
        // Đếm DN đã tiếp xúc theo từng cụm trong một region, trong tháng
        @Query("SELECT e.commune.cluster.name, COUNT(DISTINCT a.enterprise.id) "
                        + "FROM Appointment a JOIN a.enterprise e "
                        + "WHERE a.status = 'CONFIRMED' "
                        + "AND e.commune.cluster.region = :region "
                        + "AND MONTH(a.scheduledTime) = :month "
                        + "AND YEAR(a.scheduledTime) = :year " + "GROUP BY e.commune.cluster.name")
        List<Object[]> countContactedByClusterInRegion(@Param("region") RegionEnum region,
